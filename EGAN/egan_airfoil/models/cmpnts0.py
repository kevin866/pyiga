...
class BezierGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        total_params1 = sum(p.numel() for p in self.cpw_generator.parameters() if p.requires_grad)
        total_params2 = sum(p.numel() for p in self.feature_generator.parameters() if p.requires_grad)
        total_params3 = sum(p.numel() for p in self.bezier_layer.parameters() if p.requires_grad)

        print(f"cpw feature Parameters: {total_params1,total_params2,total_params3}")

...
from models.gans0 import BezierEGAN, BezierSEGAN, BezierGAN
...
def assemble_new_gan(dis_cfg, gen_cfg, egan_cfg, device='cpu'):
    ...
    egan = BezierGAN(generator, discriminator, **egan_cfg)
...
def metrics(epoch, generator, writer, *args, **kwargs):
    if (epoch + 1) % 100 == 0:
        ...
        X_test = np.load('../data/TestS3.npy')
        X = np.load('../data/TrainS3.npy')
...
if __name__ == '__main__':
    ...
    epochs = 10000 # 500 initial
    save_intvl = 500
    n_run = 1
    latent = [2]

    for i in range(len(latent)):
        dis_cfg, gen_cfg, egan_cfg, cz = read_configs('vanilla')

        # data_fname = '../data/airfoil_interp.npy'
        data_fname = '../data/TrainS3.npy'
        ...
        tb_dir = os.path.join(save_dir, 'runs', 'dim_{}'.format(latent[i]), current_time+'Bezier S3 20000 test')
